<?php
	...
	{
		$id=$_GET['id'];
		$qry1="SELECT i.*, it.ItemName FROM inventory i LEFT JOIN item it ON i.ItemID = it.ItemID WHERE i.InventoryID='$id'";
			...
			<form action="<?=$_SERVER['PHP_SELF']?>" method="post">
				...
				<p>
					...
					<select name="item" required>
						<?php
						$items_sql = "SELECT ItemID, ItemName FROM item ORDER BY ItemName";
						$items_result = $conn->query($items_sql);
						while($item = $items_result->fetch_assoc()) {
							$selected = ($item['ItemID'] == $row[1]) ? 'selected' : '';
							echo "<option value='".$item['ItemID']."' ".$selected.">".$item['ItemID']." - ".$item['ItemName']."</option>";
						}
						?>
					</select>
				</p>
